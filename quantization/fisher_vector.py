...
def gen_fv(samples, gmm_means, gmm_covs, gmm_weights):
    ...
    s0, s1, s2 = gen_likelihood(samples,
            gmm_means, gmm_covs, gmm_weights)
    T = samples.shape[0]
    sigmas = np.float32([np.diagonal(gmm_covs[k]) for k in range(0, gmm_covs.shape[0])])
    ...
    fv_w = fv_weight(s0, s1, s2, gmm_means, gmm_covs, gmm_weights, T)
    fv_m = fv_means(s0, s1, s2, gmm_means, gmm_covs, gmm_weights)
    fv_s = fv_sigma(s0, s1, s2, gmm_means, gmm_covs, gmm_weights)
    fv = np.concatenate([np.concatenate(fv_w), np.concatenate(fv_m), np.concatenate(fv_s)])
...
def gen_likelihood(samples, means, covs, weights):
    ...
    for index, x in samples:
        ...
        print "gaussians[index] shape: ", gaussians[index].shape
        s0, s1, s2 = {}, {}, {}
        for k in range(0, len(weights)):
